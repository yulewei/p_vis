...
public class ParallelDisplay extends JComponent implements ChangeListener,
		...
		if (model != null) {
			...
			for (int i = 0; i < model.getNumDimensions(); i++) {
				...
				Axis newAxis = new Axis(i, model.getMinValue(i)
						- model.getMaxValue(i), model.getMaxValue(i),
						model.getAxisLabel(i));
				axes[i] = newAxis;
	...
	public void fireBrushChanged(Brush b) {
		Vector<BrushListener> list = (Vector) brushListeners.clone();
		for (int i = 0; i < list.size(); i++) {
	...
	public void fireBrushModified(Brush b) {
		Vector<BrushListener> list = (Vector<BrushListener>) brushListeners
				.clone();
		for (int i = 0; i < list.size(); i++) {
			BrushListener l = (BrushListener) list.elementAt(i);
			l.brushModified(b);
	 ...
	 * @return
	 */
	public int[] findSelection(Brush b) {
		int numSelected = 0;
	...
	public void fireSelectionChanged(Brush b) {
		// Guaranteed to return a non-null array
		Object[] listeners = listenerList.getListenerList();
		...
		for (int i = listeners.length - 2; i >= 0; i -= 2) {
			if (listeners[i] == SelectionListener.class) {
				// Lazily create the event:
				if (e == null) {
					int[] newSelection = findSelection(b);
