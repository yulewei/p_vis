...
public class SummariseNormalisedMean extends SummariseField{
	 ...
	 */
	public SummariseNormalisedMean(String name,DataField dataField,DataField normaliseField){
		this(name,dataField,normaliseField,FieldType.FLOAT);
	}

	public SummariseNormalisedMean(String name,DataField dataField,DataField normaliseField,FieldType fieldType){
		this.name=name;
	...
	public Object compute(List<Record> records){
		...
		double sum=0;
		try{
			avg=((Number)new SummariseMean(null,dataField).compute(records)).doubleValue();
			sum=((Number)new SummariseSum(null,normaliseField).compute(records)).doubleValue();
		}
