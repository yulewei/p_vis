...
public class SummariseNode extends DefaultMutableTreeNode implements Comparable<SummariseNode> {
	...
	private static final long serialVersionUID = -2877411666911600218L;
	 ...
	 */
	public SummariseNode(DataField dataField, Object  groupByValue, int order, List<Record> sortedRecords, int startIdx,int endIdx,HashMap<SummariseField, Object> summaryField2Value) {
	...
	/** Gets the path id
	 *  A string that's the variable name, colon, value for each hierarchical level
	 *  separated by /. Root is /
	 * @return
	 */
	public String getPathId(){
		if (this.isRoot()){
			...
			while (!node.isRoot()){
				path=node.conditioningField.getName()+":"+node.conditioningValue+"/"+path;
	...
	/**Get's the node's hierarchical level
	 * -1 for the root
	 * @return level
	 */
	public int getLevel(){
	 ...
	 * 
	 * @return Will be null, String, Float, Double, Long or Integer (according to the datatype)
	 */
	...
	public Long getSummaryAsLong(SummariseField summaryField){
		try{
			return ((Number)summariseField2Value.get(summaryField)).longValue();
		}
		catch (NumberFormatException e){
			System.out.println(e.getStackTrace());
			return 0l;
	...
	public Float getSummaryAsFloat(SummariseField summaryField){
		try{
			return ((Number)summariseField2Value.get(summaryField)).floatValue();
		}
		catch (NumberFormatException e){
			System.out.println(e.getStackTrace());
			return 0f;
	...
	public Double getSummaryAsDouble(SummariseField summaryField){
		try{
			return ((Number)summariseField2Value.get(summaryField)).doubleValue();
		}
		catch (NumberFormatException e){
			System.out.println(e.getStackTrace());
			return 0d;
	...
	/** Get the value of the specified summary field as a string
	 * (slightly less efficient then passing the SummariseField directly)
	 * @param summaryField
	 * @return
	 */
	...
	public Integer getSummaryAsInt(String summaryField){
		SummariseField summaryField1 = this.getSummaryField(summaryField);
		try{
			return ((Number)summariseField2Value.get(summaryField1)).intValue();
		}
		catch (NumberFormatException e){
			System.out.println(e.getStackTrace());
			return 0;
	...
	public Long getSummaryAsLong(String summaryField){
		SummariseField summaryField1 = this.getSummaryField(summaryField);
		try{
			return ((Number)summariseField2Value.get(summaryField1)).longValue();
		}
		catch (NumberFormatException e){
			System.out.println(e.getStackTrace());
			return 0l;
	...
	public Float getSummaryAsFloat(String summaryField){
		SummariseField summaryField1 = this.getSummaryField(summaryField);
		try{
			return ((Number)summariseField2Value.get(summaryField1)).floatValue();
		}
		catch (NumberFormatException e){
			System.out.println(e.getStackTrace());
			return 0f;
	...
	public Double getSummaryAsDouble(String summaryField){
		SummariseField summaryField1 = this.getSummaryField(summaryField);
		try{
			return ((Number)summariseField2Value.get(summaryField1)).doubleValue();
		}
		catch (NumberFormatException e){
			System.out.println(e.getStackTrace());
			return 0d;
	...
	/**Returns a collection of empty summaries of any (ordered) values specifies in the datafield
	 * that does not already exist as a child.
	 * Uses the position of the ordered values as the natural order
	 * @param datafield
	 * @return
	 */
	public Collection<SummariseNode> getEmptySummariesForChildren(DataField dataFieldOfChildren){
			...
			for (Object value : dataFieldOfChildren.getOrderValues()) {
				...
				if (children!=null){
					...
					while (it.hasNext() && !exists){
						SummariseNode node = it.next();
						if ((node.getConditioningValue()!=null && node.getConditioningValue().equals(value))||
				...
				if (!exists){
					...
					returnNodes.add(new SummariseNode(dataFieldOfChildren,value,dataFieldOfChildren.getOrderValues().indexOf(value),sortedRecords,0,0,new HashMap<SummariseField, Object>()));
				}
			}
		}
		return returnNodes; 
	...
	public String toString(){
		...
		Iterator<Entry<SummariseField,Object>> it = summariseField2Value.entrySet().iterator();
		while (it.hasNext()){
