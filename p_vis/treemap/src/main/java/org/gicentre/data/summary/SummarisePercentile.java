...
public class SummarisePercentile extends SummariseField{
	...
	public SummarisePercentile(String name,DataField dataField,int percentile,FieldType fieldType){
		this.name=name;
	...
	public Object compute(List<Record> records){
		...
		for (Record record:records){
			if (useRecord(record, valuesUsed)){
				Comparable<Object> comparable=(Comparable<Object>)record.getValue(dataField);
				if (comparable!=null){
		...
		if (sortedList.isEmpty()){
			...
			value=sortedList.get((int)(sortedList.size()*(percentile/(float)100))).toString();
